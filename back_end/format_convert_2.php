...
<body>
    ...
    if (isset($_POST['content'])) {
        ...
        // explore: cat chuoi thanh mang array phan tach nhau bang \n
        $content = explode("\n", $_POST['content']);
        $text_1 = [];
        foreach ($content as $value) {
            
            // str_contains or strpos: tim tra ve vi tri ki tu dau tien trong chuoi, tra false neu ko tim thay
            if (str_contains($value, '=')) {
                $content_arr = explode('=', $value);

                // trim: loai bo khoang trang dau va cuoi chuoi
                $line_right_0 = trim($content_arr[0]);
                $line_right = trim($content_arr[1]);
                if (str_contains($line_right, '(')) {
                ...
                } elseif (str_contains($line_right, ';') == false) {
        ...
        // implode: noi phan tu mang thanh chuoi va them ki tu \n
